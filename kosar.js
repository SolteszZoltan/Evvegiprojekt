...
function setupEventListeners() {
    ...
    if (checkoutBtn) {
        checkoutBtn.addEventListener('click', async function() {
            ...
            const currentUser = JSON.parse(localStorage.getItem('pronyo_current_user') || 'null');

            if (!currentUser || !currentUser.email) {
                alert('⚠️ A rendeléshez be kell jelentkezned!');
            ...
            const order = {
                ...
                items: cart.map(i => ({ name: i.name, size: i.size || '-', qty: i.quantity || 1, price: i.price || 0 })),
                total: total,
                status: 'Új'
            };

            try {
                const orders = JSON.parse(localStorage.getItem('pronyo_orders') || '[]');
                orders.unshift(order);
                localStorage.setItem('pronyo_orders', JSON.stringify(orders));
            } catch(e) {
                console.error('Rendelés mentési hiba:', e);
            }

            // ─── EMAILJS VISSZAIGAZOLÁS ───
            try {
                const itemsText = order.items.map(i =>
                    i.name + ' | Méret: ' + i.size + ' | Mennyiség: ' + i.qty + ' db | ' + i.price.toLocaleString('hu-HU') + ' Ft'
                ).join('\n');
                ...
                await emailjs.send(
                    ...
                    'template_1uys95e',
                    {
                        ...
                        date:          order.date,
                        items:         itemsText,
                        total:         total.toLocaleString('hu-HU'),
                        to_email:      order.email
                    }
                );
                console.log('✅ Visszaigazoló email elküldve:', order.email);
            } catch(emailErr) {
            ...
            alert('Köszönjük a rendelésed! 🎉\n\nA rendelés #' + order.id + ' rögzítésre került.\nVisszaigazoló emailt küldtünk a ' + order.email + ' címre.');
        });
    }
}
